...
export default function JobPreviewPopup({ job, position, onPopupHover }: JobPreviewPopupProps) {
  const navigate = useNavigate();
  const [isSaved, setIsSaved] = useState(false);
  const company = getCompanyById(job.companyId);
  const locationName = job.locationIds ? getLocationById(job.locationIds[0])?.name : 'Chưa xác định';
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-slate-900 text-base line-clamp-2">{job.title}</h3>
          <p className="text-sm text-slate-600 mt-1 font-medium">{company?.name || 'Công ty'}</p>
        </div>
      </div>
      ...
      <div className="mb-4 pb-4 border-b border-slate-200">
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-slate-900 line-clamp-2">{job.salary}</p>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-slate-900">1 năm</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <ul className="text-xs text-slate-600 space-y-2">
          <li className="flex gap-2">
            ...
            <span>Tiếp nhận chứng từ từ phía khách hàng, kiểm tra tính hợp lệ của chứng từ</span>
          </li>
          <li className="flex gap-2">
            <span className="text-emerald-600 font-bold">•</span>
            <span>Xử lý tờ khai xuất nhập khẩu thuốc lá hàng nhà máy, hàng kinh doanh</span>
          </li>
          <li className="flex gap-2">
            <span className="text-emerald-600 font-bold">•</span>
            <span>Phân luồng tờ khai sau khi có xác nhận từ phía khách hàng</span>
          </li>
        </ul>
      </div>
      ...
      <div className="mb-4 pb-4 border-b border-slate-200">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <h4 className="font-semibold text-slate-900 text-xs mb-2">Địa điểm làm việc</h4>
          ...
          <div>
            <h4 className="font-semibold text-slate-900 text-xs mb-2">Thời gian làm việc</h4>
            <p className="text-xs text-slate-600 flex items-center gap-1">
