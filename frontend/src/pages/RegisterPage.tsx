	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (registeredUser.data.errCode === 0) {
				...
				setErrorMessage(
					registeredUser.data.message || "Đăng ký thất bại. Vui lòng thử lại.",
				);
						...
						{[
							{
								step: "01",
								title: "Tạo tài khoản",
								desc: "Đăng ký miễn phí trong 2 phút",
							},
							{
								step: "02",
								title: "Hoàn thiện hồ sơ",
								desc: "Thêm kỹ năng và kinh nghiệm của bạn",
							},
							{
								step: "03",
								title: "Ứng tuyển ngay",
								desc: "Tìm và apply hàng ngàn cơ hội",
							},
						].map((item, idx) => (
							<div key={idx} className="flex items-start gap-4">
								<span className="text-slate-600 font-bold text-xs mt-0.5 w-6 flex-shrink-0">
									{item.step}
								</span>
					...
					<div className="w-full max-w-sm">
						...
						<div className="mb-6">
							...
							<p className="text-slate-500 text-sm">
								Chọn vai trò để bắt đầu đăng ký
							</p>
						</div>
