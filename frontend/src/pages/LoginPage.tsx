...
export default function LoginPage() {
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const authenticatedUser = await loginWithScope(scope, {
				email,
				password,
			});
			if (activeTab.toLowerCase() !== authenticatedUser.role.toLowerCase()) {
				setErrorMessage("Đăng nhập thất bại. Kiểm tra lại email và mật khẩu");
				logoutSession("all");
	...
	const handleGoogleLogin = async () => {
		...
		setErrorMessage("");
		try {
			const authenticatedUser = await loginWithFirebase();
			if (activeTab.toLowerCase() !== "candidate") {
				setErrorMessage("Đăng nhập thất bại. Kiểm tra lại email và mật khẩu");
				logoutSession("all");
				setIsSubmitting(false);
				return;
			...
			setErrorMessage(error?.message || "Đăng nhập bằng Google thất bại.");
		} finally {
			setIsSubmitting(false);
		}
	};
				...
				<div className="relative z-10 space-y-6">
					<div className="space-y-2">
						...
						<h2 className="text-4xl font-bold text-white leading-tight">
							Tìm việc làm
							<br />
							<span className="text-slate-400">phù hợp nhất</span>
							<br />
							với bạn.
						</h2>
					</div>
					<p className="text-slate-400 text-sm leading-relaxed max-w-xs">
						Kết nối hàng ngàn ứng viên với nhà tuyển dụng uy tín. Hành trình sự
						nghiệp của bạn bắt đầu từ đây.
					</p>
			...
			<div className="flex-1 flex flex-col min-h-screen bg-white">
				...
				<div className="lg:hidden flex items-center justify-between px-6 py-4 border-b border-slate-100">
					<Link to="/" className="flex items-center gap-2 group">
						...
						<span className="font-bold text-slate-900">MNP</span>
					</Link>
					<Link
						to="/"
					...
					<div className="w-full max-w-sm">
						...
						{activeTab === "candidate" && (
							<button
								...
								disabled={isSubmitting}
								className="w-full h-11 mt-4 border border-slate-200 hover:border-slate-900 text-slate-700 hover:text-slate-900 font-semibold rounded-lg text-sm transition-all duration-150 flex items-center justify-center gap-2"
							>
