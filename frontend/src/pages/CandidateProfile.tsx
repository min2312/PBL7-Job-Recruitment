...
export default function CandidateProfile() {
	...
	const handlePasswordChange = async () => {
		...
		if (!passwords.current)
			return setPassError("Vui lòng nhập mật khẩu hiện tại.");
		if (passwords.newPass.length < 8)
			return setPassError("Mật khẩu mới tối thiểu 8 ký tự.");
		if (passwords.newPass !== passwords.confirm)
			return setPassError("Xác nhận mật khẩu không khớp.");
	...
	const handleCVRemove = async () => {
		...
		if (user?.cv_file) {
			setConfirmModal({
				...
				description:
					"Hệ thống sẽ xóa file CV cũ của bạn. Bạn sẽ cần tải lên file mới để nhà tuyển dụng có thể xem.",
				isLoading: false,
				onConfirm: async () => {
					setConfirmModal((prev) => ({ ...prev, isLoading: true }));
					try {
				...
				<div className="bg-card border border-border rounded-2xl p-6 -mt-12 mb-8 relative z-10 shadow-sm">
					<div className="flex items-center gap-4 flex-1">
						<div className="relative group">
							{/* Avatar Display */}
							<div
								className={`w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold text-white flex-shrink-0 transition-all group-hover:ring-2 group-hover:ring-blue-400 ${
									user?.profilePicture
										? "bg-cover bg-center"
										: profile.name.toLowerCase().includes("a")
											? "bg-blue-500"
...
function Section({
	...
	return (
		<div
			className={`bg-card border border-border rounded-2xl p-5 md:p-6 mb-6 ${className || ""}`}
		>
