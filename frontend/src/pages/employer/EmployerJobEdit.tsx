...
export default function EmployerJobEdit({ refreshData }: Props) {
	const { id } = useParams();
	...
	useEffect(() => {
		const fetchData = async () => {
			setLoading(true);
			try {
				const [jobRes, catRes, locRes] = await Promise.all([
				...
				if (jobRes.data.errCode === 0) {
					...
					setForm({
						...
						age: job.age ? String(job.age) : "",
						experience: job.experience || "Không yêu cầu",
						education: job.education || "Không yêu cầu",
						employmentType: job.employmentType || "Toàn thời gian",
						...
						locationIds: job.locations?.map((l: any) => l.id) || [],
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const res = await axiosClient.put(`/api/jobs/update/${id}`, payload);
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								...
								<Select
									value={form.locationIds[0]?.toString()}
									onValueChange={(val) =>
										setForm((prev) => ({
											...prev,
											locationIds: [parseInt(val)],
										}))
									}
								>
									<SelectTrigger className="h-9">
							...
							<div className="space-y-2">
								...
								>
									...
									<SelectContent>
										...
										{form.experience && (
											<SelectItem value={form.experience}>
												{form.experience}
											</SelectItem>
										)}
									</SelectContent>
							...
							<div className="space-y-2">
								...
								>
									...
									<SelectContent>
										...
										{form.education && (
											<SelectItem value={form.education}>
												{form.education}
											</SelectItem>
										)}
									</SelectContent>
