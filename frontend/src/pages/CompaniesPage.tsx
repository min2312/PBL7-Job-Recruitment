...
export default function CompaniesPage() {
  ...
  // Memoize paginated companies
  const paginatedCompanies = useMemo(() => 
    filteredCompanies.slice(startIndex, startIndex + itemsPerPage),
  ...
  useEffect(() => {
    if (companiesGridRef.current) {
      companiesGridRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }, [currentPage]);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-emerald-50 py-12">
        <div className="max-w-7xl mx-auto px-12">
          <h1 className="text-4xl font-bold text-slate-900 mb-4">Khám phá 100.000+ công ty nổi bật</h1>
          <p className="text-lg text-slate-600 mb-8">Tra cứu thông tin công ty và tìm kiếm nơi làm việc tốt nhất dành cho bạn</p>
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button className="bg-black hover:bg-slate-800 text-white px-6 py-3 rounded-lg font-semibold">
              Tìm kiếm
            </Button>
          </div>
        </div>
      ...
      <div ref={companiesGridRef} className="max-w-7xl mx-auto px-12 py-12">
        <h2 className="text-2xl font-bold text-slate-900 mb-8 uppercase tracking-wide">Danh sách các công ty nổi bật</h2>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedCompanies.map(company => {
            // Get the first job from this company to use its description
            const companyJob = jobs.find(job => job.companyId === company.id);
            const description = companyJob?.description || 'Công ty chuyên cung cấp giải pháp tuyển dụng hàng đầu.';
        ...
        {paginatedCompanies.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500 text-lg">Không tìm thấy công ty phù hợp</p>
          </div>
        )}
