...
const extractAuthPayload = (
	responseData: AuthResponseRecord | null | undefined,
): unknown => {
...
const getAccountPath = (scope: AuthScope) =>
	scope === "admin" ? "/api/admin/account" : "/api/account";
const getLoginPath = (scope: AuthScope) =>
	scope === "admin" ? "/api/admin/login" : "/api/login";
const getRefreshPath = (scope: AuthScope) =>
	scope === "admin" ? "/api/admin/refresh-token" : "/api/refresh-token";
const getLogoutPath = (scope: AuthScope) =>
	scope === "admin" ? "/api/admin/logout" : "/api/logout";
...
export async function loginWithScope(
	...
	if (responseCode !== 0) {
		const error = new Error(
			responseData.message || responseData.errMessage || "Đăng nhập thất bại",
		) as Error & {
...
export async function loginWithFirebase() {
	...
	const response = await authHttpClient.post("/api/firebase-login", {
		idToken,
	});
	const responseData = response.data as AuthResponseRecord;
	const responseCode = responseData.errCode ?? responseData.errcode ?? 0;
...
export async function removeFile(type: "avatar" | "cv") {
	...
	if (responseData.errCode !== 0) {
		throw new Error(
			responseData.message || responseData.errMessage || "Xóa thất bại",
		);
	}
	return responseData;
}
...
export async function logoutSession(scope: AuthScope | "all" = "all") {
	...
	await Promise.allSettled(
		scopes.map((item) => authHttpClient.post(getLogoutPath(item))),
	);
}
